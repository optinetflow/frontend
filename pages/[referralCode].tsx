...
const ReferralCodePage: NextPageWithLayout = () => {
  const router = useRouter()
  const password = router.query?.referralCode as string
  ...
  if (loginData?.data?.login) {
    if (loginData?.data?.login?.isPromoCodeValid) {
      ...
      router.push(`/signup?phone=${formData.phone}&promoCode=${password}`);
      return;
    }
  ...
  ReferralCodePage.getLayout = function getLayout(page: React.ReactElement) {
